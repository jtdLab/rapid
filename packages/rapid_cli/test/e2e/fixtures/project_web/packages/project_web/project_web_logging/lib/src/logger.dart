...
/// The log level of an [ProjectWebLogger].
enum Level {
  verbose(0),
  debug(500),
  info(800),
...
abstract class ProjectWebLogger {
  final Level level;

  @visibleForTesting
  LogCommand? logCommandOverrides;

  ProjectWebLogger(this.level);
...
@dev
@LazySingleton(as: ProjectWebLogger)
class ProjectWebLoggerDevelopment extends ProjectWebLogger {
  ProjectWebLoggerDevelopment() : super(Level.debug);
}
...
@test
@LazySingleton(as: ProjectWebLogger)
class ProjectWebLoggerTest extends ProjectWebLogger {
  ProjectWebLoggerTest() : super(Level.verbose);
}

/// In production mode we log nothing for security reasons.
@prod
@LazySingleton(as: ProjectWebLogger)
class ProjectWebLoggerProduction extends ProjectWebLogger {
  ProjectWebLoggerProduction() : super(Level.debug);
}
