...
const expectedUsage = [
  'Adds a cubit to a feature of the Web part of an existing Rapid project.\n'
      '\n'
      'Usage: rapid web feature add cubit <name> [arguments]\n'
...
void main() {
  group('web feature add cubit', () {
    ...
    late WebFeatureAddCubitCommand command;
    ...
    setUp(() {
      ...
      command = WebFeatureAddCubitCommand(
        logger: logger,
        project: project,
    ...
    test('can be instantiated without explicit logger', () {
      // Act
      command = WebFeatureAddCubitCommand(project: project);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        final result = await commandRunner
            .run(['web', 'feature', 'add', 'cubit', '--feature', 'some_feat']);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result = await commandRunner.run(
            ['web', 'feature', 'add', 'cubit', name, '--feature', featureName]);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result = await commandRunner
            .run(['web', 'feature', 'add', 'cubit', 'FooBar']);
    ...
    test('completes successfully with correct output', () async {
      ...
      verify(
        () => logger.success(
          'Added ${name.pascalCase}Cubit to Web feature $featureName.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the cubit already exists for the feature',
      ...
      verify(
        () => logger.err(
          'The cubit $name does already exist in $featureName on Web.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
      expect(result, ExitCode.config.code);
