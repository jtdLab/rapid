...
/// The log level of an [TestAppLogger].
enum Level {
  verbose(0),
  debug(500),
  info(800),
...
abstract class TestAppLogger {
  final Level level;

  @visibleForTesting
  LogCommand? logCommandOverrides;

  TestAppLogger(this.level);
...
@dev
@LazySingleton(as: TestAppLogger)
class TestAppLoggerDevelopment extends TestAppLogger {
  TestAppLoggerDevelopment() : super(Level.debug);
}
...
@test
@LazySingleton(as: TestAppLogger)
class TestAppLoggerTest extends TestAppLogger {
  TestAppLoggerTest() : super(Level.verbose);
}

/// In production mode we log nothing for security reasons.
@prod
@LazySingleton(as: TestAppLogger)
class TestAppLoggerProduction extends TestAppLogger {
  TestAppLoggerProduction() : super(Level.debug);
}
