...
/// The log level of an [ProjectIosLogger].
enum Level {
  verbose(0),
  debug(500),
  info(800),
...
abstract class ProjectIosLogger {
  final Level level;

  @visibleForTesting
  LogCommand? logCommandOverrides;

  ProjectIosLogger(this.level);
...
@dev
@LazySingleton(as: ProjectIosLogger)
class ProjectIosLoggerDevelopment extends ProjectIosLogger {
  ProjectIosLoggerDevelopment() : super(Level.debug);
}
...
@test
@LazySingleton(as: ProjectIosLogger)
class ProjectIosLoggerTest extends ProjectIosLogger {
  ProjectIosLoggerTest() : super(Level.verbose);
}

/// In production mode we log nothing for security reasons.
@prod
@LazySingleton(as: ProjectIosLogger)
class ProjectIosLoggerProduction extends ProjectIosLogger {
  ProjectIosLoggerProduction() : super(Level.debug);
}
