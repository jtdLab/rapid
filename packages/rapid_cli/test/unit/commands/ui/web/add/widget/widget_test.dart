...
const expectedUsage = [
  'Add a widget to the Web UI part of an existing Rapid project.\n'
      '\n'
      'Usage: rapid ui web add widget <name> [arguments]\n'
...
void main() {
  group('ui web add widget', () {
    ...
    test('can be instantiated without explicit logger', () {
      // Act
      command = UiWebAddWidgetCommand(project: project);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result = await commandRunner.run(['ui', 'web', 'add', 'widget']);
    ...
    test('exits with 78 when the referenced widget already exists', () async {
      ...
      // Assert
      verify(
        () => logger.err('Web Widget $name already exists.'),
      ).called(1);
      verify(() => logger.info('')).called(1);
      expect(result, ExitCode.config.code);
    });
