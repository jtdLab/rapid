...
/// The log level of an [ProjectMacosLogger].
enum Level {
  verbose(0),
  debug(500),
  info(800),
...
abstract class ProjectMacosLogger {
  final Level level;

  @visibleForTesting
  LogCommand? logCommandOverrides;

  ProjectMacosLogger(this.level);
...
@dev
@LazySingleton(as: ProjectMacosLogger)
class ProjectMacosLoggerDevelopment extends ProjectMacosLogger {
  ProjectMacosLoggerDevelopment() : super(Level.debug);
}
...
@test
@LazySingleton(as: ProjectMacosLogger)
class ProjectMacosLoggerTest extends ProjectMacosLogger {
  ProjectMacosLoggerTest() : super(Level.verbose);
}

/// In production mode we log nothing for security reasons.
@prod
@LazySingleton(as: ProjectMacosLogger)
class ProjectMacosLoggerProduction extends ProjectMacosLogger {
  ProjectMacosLoggerProduction() : super(Level.debug);
}
