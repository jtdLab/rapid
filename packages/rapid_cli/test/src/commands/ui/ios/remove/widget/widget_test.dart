...
const expectedUsage = [
  'Remove a widget from the iOS UI part of an existing Rapid project.\n'
      '\n'
      'Usage: rapid ui ios remove widget <name> [arguments]\n'
...
void main() {
  group('ui ios remove widget', () {
    ...
    late UiIosRemoveWidgetCommand command;
    ...
    setUp(() {
      ...
      command = UiIosRemoveWidgetCommand(
        logger: logger,
        project: project,
    ...
    test('can be instantiated without explicit logger', () {
      // Act
      command = UiIosRemoveWidgetCommand(project: project);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result =
            await commandRunner.run(['ui', 'ios', 'remove', 'widget']);
    ...
    test('completes successfully with correct output', () async {
      ...
      // Assert
      verify(() => logger.info('Removing iOS Widget ...')).called(1);
      verify(
        () => project.removeWidget(
          name: name,
          dir: '.',
      ...
      verify(() => logger.success('Removed iOS Widget $name.')).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the referenced widget does not exist', () async {
      ...
      // Assert
      verify(() => logger.err('iOS Widget $name not found.')).called(1);
      verify(() => logger.info('')).called(1);
      expect(result, ExitCode.config.code);
    });
  });
