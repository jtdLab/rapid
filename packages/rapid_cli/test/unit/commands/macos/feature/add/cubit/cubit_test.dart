...
void main() {
  group('macos feature add cubit', () {
    ...
    late MacosFeatureAddCubitCommand command;
    ...
    test('can be instantiated without explicit logger', () {
      // Act
      command = MacosFeatureAddCubitCommand(project: project);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        final result = await commandRunner
            .run(['macos', 'feature', 'add', 'cubit', 'FooBar']);
    ...
    test('completes successfully with correct output', () async {
      ...
      verify(
        () => logger.success(
          'Added ${name.pascalCase}Cubit to macOS feature $featureName.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the cubit already exists for the feature',
      ...
      verify(
        () => logger.err(
          'The cubit $name does already exist in $featureName on macOS.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
      expect(result, ExitCode.config.code);
