...
const expectedUsage = [
  'Adds a cubit to a feature of the iOS part of an existing Rapid project.\n'
      '\n'
      'Usage: rapid ios feature add cubit <name> [arguments]\n'
...
void main() {
  group('ios feature add cubit', () {
    ...
    late IosFeatureAddCubitCommand command;
    ...
    setUp(() {
      ...
      command = IosFeatureAddCubitCommand(
        logger: logger,
        project: project,
    ...
    test('can be instantiated without explicit logger', () {
      // Act
      command = IosFeatureAddCubitCommand(project: project);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        final result = await commandRunner
            .run(['ios', 'feature', 'add', 'cubit', '--feature', 'some_feat']);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result = await commandRunner.run(
            ['ios', 'feature', 'add', 'cubit', name, '--feature', featureName]);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result = await commandRunner
            .run(['ios', 'feature', 'add', 'cubit', 'FooBar']);
    ...
    test('completes successfully with correct output', () async {
      ...
      verify(
        () => logger.success(
          'Added ${name.pascalCase}Cubit to iOS feature $featureName.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the feature does not exists', () async {
      ...
      verify(
        () => logger.err(
          'The feature $featureName does not exist on iOS.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the cubit already exists for the feature',
      ...
      verify(
        () => logger.err(
          'The cubit $name does already exist in $featureName on iOS.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
      expect(result, ExitCode.config.code);
