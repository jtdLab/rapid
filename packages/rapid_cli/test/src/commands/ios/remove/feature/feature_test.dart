...
const expectedUsage = [
  'Removes a feature from the iOS part of an existing Rapid project.\n'
      '\n'
      'Usage: rapid ios remove feature <name>\n'
...
void main() {
  group('ios remove feature', () {
    ...
    late IosRemoveFeatureCommand command;
    ...
    setUp(() {
      ...
      command = IosRemoveFeatureCommand(
        logger: logger,
        project: project,
    ...
    test('feat is a valid alias', () {
      // Arrange
      command = IosRemoveFeatureCommand(project: project);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result = await commandRunner.run(['ios', 'remove', 'feature']);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        final result = await commandRunner
            .run(['ios', 'remove', 'feature', 'name1', 'name2']);
    ...
    test('completes successfully with correct output', () async {
      ...
      verify(
        () => logger.success('Removed iOS feature $name.'),
      ).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the feature does not exist', () async {
      ...
      // Assert
      verify(
        () => logger.err('The feature "$name" does not exist on iOS.'),
      ).called(1);
      verify(() => logger.info('')).called(1);
