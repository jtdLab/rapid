// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, implicit_dynamic_list_literal, implicit_dynamic_map_literal, inference_failure_on_collection_literal

import 'package:mason/mason.dart';

final languageBundle = MasonBundle.fromJson(<String, dynamic>{
  "files": [
    {"path": "HELLO.md", "data": "SGVsbG8ge3tuYW1lfX0h", "type": "text"},
    {
      "path":
          "lib/src/presentation/l10n/arb/{{feature_name.snakeCase()}}_{{language}}.arb",
      "data": "ewogICJAQGxvY2FsZSI6ICJ7e2xhbmd1YWdlfX0iCn0=",
      "type": "text"
    }
  ],
  "hooks": [],
  "name": "language",
  "description": "A new brick created with the Mason CLI.",
  "version": "0.1.0+1",
  "environment": {"mason": ">=0.1.0-dev.39 <0.1.0"},
  "readme": {
    "path": "README.md",
    "data":
        "",
    "type": "text"
  },
  "changelog": {
    "path": "CHANGELOG.md",
    "data": "IyAwLjEuMCsxCgotIFRPRE86IERlc2NyaWJlIGluaXRpYWwgcmVsZWFzZS4K",
    "type": "text"
  },
  "license": {
    "path": "LICENSE",
    "data": "VE9ETzogQWRkIHlvdXIgbGljZW5zZSBoZXJlLgo=",
    "type": "text"
  },
  "vars": {
    "feature_name": {
      "type": "string",
      "description": "The name of the feature this language is added to"
    },
    "language": {"type": "string", "description": "The language that is added"}
  }
});
