...
const expectedUsage = [
  'Remove a widget from the macOS UI part of an existing Rapid project.\n'
      '\n'
      'Usage: rapid ui macos remove widget <name> [arguments]\n'
...
void main() {
  group('ui macos remove widget', () {
    ...
    late UiMacosRemoveWidgetCommand command;
    ...
    setUp(() {
      ...
      command = UiMacosRemoveWidgetCommand(
        logger: logger,
        project: project,
    ...
    test('can be instantiated without explicit logger', () {
      // Act
      command = UiMacosRemoveWidgetCommand(project: project);
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result =
            await commandRunner.run(['ui', 'macos', 'remove', 'widget']);
    ...
    test('completes successfully with correct output', () async {
      ...
      // Assert
      verify(() => logger.info('Removing macOS Widget ...')).called(1);
      verify(
        () => project.removeWidget(
          name: name,
          dir: '.',
      ...
      verify(() => logger.success('Removed macOS Widget $name.')).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the referenced widget does not exist', () async {
      ...
      // Assert
      verify(() => logger.err('macOS Widget $name not found.')).called(1);
      verify(() => logger.info('')).called(1);
      expect(result, ExitCode.config.code);
    });
  });
