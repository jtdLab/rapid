...
final traceColor = AnsiStyles.gray;
...
final xMarkLabel = AnsiStyles.redBright('✗');
...
/// CLI logger that encapsulates Rapid log formatting conventions.
class RapidLogger with _DelegateLogger {
  ...
  void command(String command, {bool withDollarSign = false}) {
    if (withDollarSign) {
      ...
      final style = AnsiStyles.yellow.bold;

      stdout(commandColor(commandStyle('$command ${style('⚡')}')));
    }
  }
  ...
  RapidLogger childWithoutMessage({String childIndentation = '  '}) =>
      RapidLogger(
        _logger,
        indentation: '$_indentation$_childIndentation',
        childIndentation: childIndentation,
  ...
  void trace(String message) =>
      _logger.trace(traceColor('$_indentation$message'));
}
...
extension ToRapidLoggerExtension on Logger {
  RapidLogger toRapidLogger() {
    if (this is RapidLogger) {
      return this as RapidLogger;
    }
    return RapidLogger(this);
  }
}
