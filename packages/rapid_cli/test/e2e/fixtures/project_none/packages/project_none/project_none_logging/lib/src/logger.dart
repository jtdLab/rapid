...
/// The log level of an [ProjectNoneLogger].
enum Level {
  verbose(0),
  debug(500),
  info(800),
...
abstract class ProjectNoneLogger {
  final Level level;

  @visibleForTesting
  LogCommand? logCommandOverrides;

  ProjectNoneLogger(this.level);
...
@dev
@LazySingleton(as: ProjectNoneLogger)
class ProjectNoneLoggerDevelopment extends ProjectNoneLogger {
  ProjectNoneLoggerDevelopment() : super(Level.debug);
}
...
@test
@LazySingleton(as: ProjectNoneLogger)
class ProjectNoneLoggerTest extends ProjectNoneLogger {
  ProjectNoneLoggerTest() : super(Level.verbose);
}

/// In production mode we log nothing for security reasons.
@prod
@LazySingleton(as: ProjectNoneLogger)
class ProjectNoneLoggerProduction extends ProjectNoneLogger {
  ProjectNoneLoggerProduction() : super(Level.debug);
}
