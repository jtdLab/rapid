  ...
  group('android feature add cubit', () {
    ...
    test(
      ...
      withRunnerOnProject(
        ...
        // Act
        final result = await commandRunner.run(
            ['android', 'feature', 'add', 'cubit', '--feature', 'some_feat']);
    ...
    test('completes successfully with correct output', () async {
      ...
      verify(
        () => logger.success(
          'Added ${name.pascalCase}Cubit to Android feature $featureName.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
    ...
    test('exits with 78 when the cubit already exists for the feature',
      ...
      verify(
        () => logger.err(
          'The cubit $name does already exist in $featureName on Android.',
        ),
      ).called(1);
      verify(() => logger.info('')).called(1);
      expect(result, ExitCode.config.code);
