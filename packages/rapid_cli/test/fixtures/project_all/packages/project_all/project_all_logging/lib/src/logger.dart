...
/// The log level of an [ProjectAllLogger].
enum Level {
  verbose(0),
  debug(500),
  info(800),
...
abstract class ProjectAllLogger {
  final Level level;

  @visibleForTesting
  LogCommand? logCommandOverrides;

  ProjectAllLogger(this.level);
...
@dev
@LazySingleton(as: ProjectAllLogger)
class ProjectAllLoggerDevelopment extends ProjectAllLogger {
  ProjectAllLoggerDevelopment() : super(Level.debug);
}
...
@test
@LazySingleton(as: ProjectAllLogger)
class ProjectAllLoggerTest extends ProjectAllLogger {
  ProjectAllLoggerTest() : super(Level.verbose);
}

/// In production mode we log nothing for security reasons.
@prod
@LazySingleton(as: ProjectAllLogger)
class ProjectAllLoggerProduction extends ProjectAllLogger {
  ProjectAllLoggerProduction() : super(Level.debug);
}
