...
/// The log level of an [ExampleLogger].
enum Level {
  verbose(0),
  debug(500),
  info(800),
...
abstract class ExampleLogger {
  final Level level;

  @visibleForTesting
  LogCommand? logCommandOverrides;

  ExampleLogger(this.level);
...
@dev
@LazySingleton(as: ExampleLogger)
class ExampleLoggerDevelopment extends ExampleLogger {
  ExampleLoggerDevelopment() : super(Level.debug);
}
...
@test
@LazySingleton(as: ExampleLogger)
class ExampleLoggerTest extends ExampleLogger {
  ExampleLoggerTest() : super(Level.verbose);
}

/// In production mode we log nothing for security reasons.
@prod
@LazySingleton(as: ExampleLogger)
class ExampleLoggerProduction extends ExampleLogger {
  ExampleLoggerProduction() : super(Level.debug);
}
